...
import engineextended.Employee.Manager;
...
//	}
	...
	private boolean checkIfEmpIsInDepartment(Employee emp){
		for (List<Employee> emps : this.employeesByDepartment.values()) {
	...
	public void addEmployeeToDepartment(String department, Employee em){
		...
		if (checkIfEmpIsInDepartment(em) && (em instanceof Manager)) {
			this.employeesByDepartment.get(department).add(em);
			return;
		}
		if (!checkIfEmpIsInDepartment(em)) {
			this.employeesByDepartment.get(department).add(em);
			return;
		}
		System.out.println("Worker("+em.getName()+"("+em.getAge()+")"+") is present in other department");
//		this.employees.add(em);
	}
	...
	public void showAllEmployeesByDepartments(){
		...
		//TODO put some css here
 		for(Map.Entry<String, ArrayList<Employee>> entry: employeesByDepartment.entrySet()){
			...
			for (Employee em : entry.getValue()) {
				System.out.println("\t"+em);
			}
		}
	}
